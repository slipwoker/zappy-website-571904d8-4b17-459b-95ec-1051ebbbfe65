...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
      ...
      const currentSupplements = document.getElementById('currentSupplements').value;
...
גיל: ${age || 'לא צוין'}
מטרות: ${goals || 'לא צוין'}
תוספי תזונה נוכחיים: ${currentSupplements || 'לא צוין'}
מצבים רפואיים: ${healthConditions || 'לא צוין'}
          `.trim();
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '571904d8-4b17-459b-95ec-1051ebbbfe65',
              name: fullName,
              email: email,
              subject: 'שאלון ייעוץ תזונתי - Morenergy',
      ...
      alert('תודה! השאלון נשלח בהצלחה. ניצור איתך קשר בקרוב.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Morenergy משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
